...
// ---------------------------------------------------------------------------
// Projects Store — in-memory, hydrated from server API
// ---------------------------------------------------------------------------
// IndexedDB persistence and syncToServer have been removed. The store loads
// from the REST API on startup (reloadFromServer) and receives live updates
// via WebSocket. Entity mutations still update the local blob for backward
// compatibility; new entity stores (issuesStore, pagesStore, etc.) talk
// directly to per-entity REST endpoints.
// ---------------------------------------------------------------------------
  ...
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      restoreProject: (id) => {
        ...
        const nameConflict = active.some((p) => p.name.toLowerCase() === project.name.toLowerCase())
      ...
      deleteIssue: (projectId, issueId) => {
        ...
        if (deletedIssue) {
          useActivityStore
            .getState()
            .logActivity(projectId, createIssueActivity(ACTIVITY_TYPES.ISSUE_DELETED, deletedIssue))
        }
      },
      ...
      updatePage: (projectId, pageId, updates) => {
        ...
        if (oldPage) {
          ...
          if (updates.status === 'published' && oldPage.status !== 'published') {
            ...
            useActivityStore
              .getState()
              .logActivity(projectId, createPageActivity(ACTIVITY_TYPES.PAGE_UPDATED, updatedPage))
          }
        }
      },
      ...
      deletePhase: (projectId, phaseId) => {
        ...
        if (deletedPhase) {
          useActivityStore
            .getState()
            .logActivity(projectId, createPhaseActivity(ACTIVITY_TYPES.PHASE_DELETED, deletedPhase))
        }
      },
      ...
      updateMilestone: (projectId, milestoneId, updates) => {
        set((state) => {
          ...
          const milestoneIndex = project.timeline.milestones.findIndex((m) => m.id === milestoneId)
          if (milestoneIndex !== -1) {
...
// ---------------------------------------------------------------------------
// Server Sync: pull project data from REST API + listen for external writes
// ---------------------------------------------------------------------------
// No more pushing local state to the server. The store is read-only from the
// server's perspective — mutations go through entity-specific REST endpoints.
// ---------------------------------------------------------------------------
...
export function reloadFromServer() {
  ...
  _reloadPromise = fetch('/api/projects')
    ...
    .then(async (summaries) => {
      ...
      let projects = fullProjects.filter(Boolean)
      if (projects.length > 0) {
        // Migrate seed project if needed
        await _seedReady
        projects = migrateSeedProject(projects)
...
// Initial load: pull latest from server + connect WebSocket
//
...
// No more IndexedDB hydration — the server is the single source of truth.
{
  reloadFromServer()
  connectWebSocket()
}
