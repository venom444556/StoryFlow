...
async function simulateNodeExecution(node, inputData) {
  ...
  switch (node.type) {
    ...
    case 'api':
      if (Math.random() < 0.2) {
        throw new Error(`API call failed: ${node.config?.url || 'No URL configured'}`)
      }
      result = {
        ...inputData,
    ...
    case 'code':
      if (inputData.apiData && node.config?.errorOnEmpty && inputData.apiData.count === 0) {
...
export async function executeWorkflow(
  ...
  const cascadeError = (fromNodeId) => {
    ...
    downstream.forEach((downNode) => {
      if (!failed.has(downNode.id) && !executed.has(downNode.id)) {
        ...
        onLog(`${downNode.title} skipped due to upstream failure`, 'warning')
  ...
  while (queue.length > 0) {
    ...
    const parentConnections = connections.filter((conn) => conn.to === node.id)
    const allParentsFinished = parentConnections.every(
      (conn) => executed.has(conn.from) || failed.has(conn.from)
    )
    const anyParentFailed = parentConnections.some((conn) => failed.has(conn.from))
  ...
  // Summary
  if (failed.size > 0) {
    onLog(`Workflow completed with ${failed.size} failure${failed.size > 1 ? 's' : ''}.`, 'error')
  } else {
